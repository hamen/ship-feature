...
- Iterate only for a **Blocker** or a **qualifying** Should-fix — a material problem of correctness,
  ...
  round is the full quorum again, on the SHA that will merge. Exit `0` means everyone ran, NOT that
  the reviews are clean — and a **benched** (out-of-quota) reviewer still exits `0`, so a benched seat
