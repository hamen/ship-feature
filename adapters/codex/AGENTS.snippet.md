...
- Cross-review with `ship-feature relay --author codex --reviewers <your agents>` (explicit list = the
  agents you have, e.g. claude,codex,cursor); keep iterating while any Blocker/Should-fix remains; exit
  `0` from the relay means everyone ran, NOT that the reviews are clean.
